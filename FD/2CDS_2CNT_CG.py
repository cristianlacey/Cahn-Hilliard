...
order centered difference in space and second order Crank-Nicolson in time.
"""
import numpy as np
...
def update(phi,dt,lap,A,tol=1e-6):
    ...
    in space and second order Crank-Nicolson in time.
        Args:
    ...
    while np.amax(np.absolute(phi_n - phi_p)) > tol:
        b = phi + dt*(lap.dot(np.power(phi_n,3)) + rhs)/2
...
# Precompute A_inv
A = np.eye(N*N) + dt*(lap + lap.dot(lap))/2
