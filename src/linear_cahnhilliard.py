...
class LinearCahnHilliard():
    '''
    Class to solve the Cahn-Hilliard equation w/o the c^3 term. Objects are
    instantiated with unpacked dictionary of input parameters. Output is then
    simply generated by invoking the .run() method of the created object.
    '''
    ...
    def step(self,phi,dt,lap,spatial_method,time_method,B,tol):
        ...
        if spatial_method == 'spectral':
            ...
            phi_n = self.backward_euler(phi,lap,dt,B,tol)

        elif time_method == '2CN':
            phi_n = self.crank_nicolson(phi,lap,dt,B,tol)
    ...
    @staticmethod
    def backward_euler(phi,lap,dt,B,tol):
        '''
    ...
    def crank_nicolson(self,phi,lap,dt,B,tol):
        '''
        ...
        b = phi + dt*(self.calc_rhs(phi,lap))/2
        phi_n = la.cg(B,b,x0=phi)[0]
        return phi_n
    ...
    def calc_rhs(phi,lap):
        ...
        '''
        return lap.dot(- phi - (lap.dot(phi)))
...
'tsteps' : 1001, # number of timesteps
...
'time_method' : '2CN', # Choice of 1FE, RK4, 1BE, 2CN
